...
contract PairYearn is ISwappaPairV1 
 {
	...
	function parseDataDeposit(bytes memory data) private pure returns (address poolAddr, uint8 inputType) {
    ...
    function swap(
        ...
        if (inputType == 1) {
            ...
            uint _before = ERC20(output).balanceOf(address(this));
            // Withdraw from yVault the balance amount of AToken
            IYearnVault(poolAddr).withdraw(inputAmount);

            uint _after = ERC20(output).balanceOf(address(this));
            uint balanceAmount = _after.sub(_before);
            ...
            require(ERC20(input).approve(poolAddr, inputAmount), "PairYearn: Approval in deposit failed!");
            // Deposit into the yVault
            IYearnVault(poolAddr).deposit(inputAmount);

             uint balance = ERC20(output).balanceOf(address(this));
