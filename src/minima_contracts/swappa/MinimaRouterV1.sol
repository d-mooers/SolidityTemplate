...
contract MinimaRouterV1 is ISwappaRouterV1, Ownable {
	...
	function swapMultiExactInputForOutputNativeIn(MultiSwapPayload calldata details, address wrappedAddr) external payable override returns (uint256[] memory) {
		...
		for(uint256 i=0; i<details.path.length; i++){
			...
			require(
				ERC20(wrappedAddr).transferFrom(address(this), details.pairs[i][0], details.inputAmounts[i]),
				"SwappaRouter: Wrapped transferfrom failed!"
			);
		
			usedWrapped += details.inputAmounts[i];
			require(usedWrapped <= msg.value, "SwappaRouter: Attempt to use more wrapped token the provided native token.");
			
			for (uint j; j < details.pairs[i].length; j++) {
				(address pairInput, address pairOutput) = (details.path[i][j], details.path[i][j + 1]);
				address next = j < details.pairs[i].length - 1 ? details.pairs[i][j+1] : (details.forwardTo[i] == 0 ? address(this) : details.pairs[details.forwardTo[i]][0]);
				bytes memory data = details.extras[i][j];
				ISwappaPairV1(details.pairs[i][j]).swap(pairInput, pairOutput, next, data);				
			}

			uint256 partnerId = getPartnerInfo(details.partner, details.deadline, details.path[i][0], output, details.sig);
			uint256 outputAmount = disperseWithFee(output, details.minOutputAmount, details.expectedOutputAmount, ERC20(output).balanceOf(address(this)) - outputBalanceBefore, details.to, partnerId);
			emit Swap(msg.sender, details.to, details.path[i][0], output, details.inputAmounts[i], outputAmount, partnerId);			
		}
	}

	function swapMultiExactInputForOutputNativeOut(MultiSwapPayload calldata details, address wrappedAddr) external override returns (uint256[] memory) {
		require(details.deadline >= block.timestamp, 'SwappaRouter: Expired!');
		require(details.path.length == details.pairs.length , "SwappaRouter: Path and Pairs mismatch!");
		require(details.pairs.length == details.extras.length, "SwappaRouter: Pairs and Extras mismatch!");
		require(details.pairs.length > 0, "SwappaRouter: Must have at least one pair!");
		...
		for(uint256 i=0; i<details.path.length; i++){
			uint256 wrappedBalanceBefore = ERC20(wrappedAddr).balanceOf(address(this));
			...
			uint256 wrappedBalanceAfter = ERC20(wrappedAddr).balanceOf(address(this));
			
			uint256 partnerId = getPartnerInfo(details.partner, details.deadline, details.path[i][0], wrappedAddr, details.sig);
			...
			uint256 outputAmount = disperseWithFee(wrappedAddr, details.minOutputAmount, details.expectedOutputAmount, wrappedBalanceAfter - wrappedBalanceBefore, address(this), partnerId);
			withdrawAmount += outputAmount;
			emit Swap(msg.sender, details.to, details.path[i][0], wrappedAddr, details.inputAmounts[i], outputAmount, partnerId);			
		}
	...
	function swapMultiExactInputForOutput(MultiSwapPayload calldata details) external override returns (uint256[] memory) {
		...
		for(uint256 i=0; i<details.path.length; i++){
			...
			address output = details.path[i][details.path[i].length - 1];
			uint256 outputBalanceBefore = ERC20(output).balanceOf(address(this));
			address receiver = details.forwardTo[i] == 0 ? address(this) : details.pairs[details.forwardTo[i]][0];
			
			for (uint j; j < details.pairs[i].length; j++) {
				(address pairInput, address pairOutput) = (details.path[i][j], details.path[i][j + 1]);
				address next = j < details.pairs[i].length - 1 ? details.pairs[i][j+1] : receiver;
				bytes memory data = details.extras[i][j];
				ISwappaPairV1(details.pairs[i][j]).swap(pairInput, pairOutput, next, data);
	...
	function swapExactInputForOutputNativeIn(SwapPayload calldata details, address wrappedAddr) external payable override ensure(details.deadline) returns (uint256 outputAmount) {
		require(details.path.length == details.pairs.length + 1 , "SwappaRouter: Path and Pairs mismatch!");
		require(details.pairs.length == details.extras.length, "SwappaRouter: Pairs and Extras mismatch!");
		require(details.pairs.length > 0, "SwappaRouter: Must have at least one pair!");
		...
		ERC20(wrappedAddr).transferFrom(address(this), details.pairs[0], wrappedBalanceAfter - wrappedBalanceBefore);
	...
	function swapExactInputForOutputNativeOut(SwapPayload calldata details, address wrappedAddr) external override ensure(details.deadline) returns (uint256 outputAmount) {
		require(details.path.length == details.pairs.length + 1 , "SwappaRouter: Path and Pairs mismatch!");
		require(details.pairs.length == details.extras.length, "SwappaRouter: Pairs and Extras mismatch!");
		require(details.pairs.length > 0, "SwappaRouter: Must have at least one pair!");
		...
		uint256 tradeOutput = ERC20(wrappedAddr).balanceOf(address(this)) - wrappedBalanceBefore;
		uint256 partnerId = getPartnerInfo(details.partner, details.deadline, details.path[0], output, details.sig);
		//Capture the slippage in the wrapped token, not the native.
		outputAmount = disperseWithFee(wrappedAddr, details.minOutputAmount, details.expectedOutputAmount, tradeOutput, address(this), partnerId);
		emit Swap(msg.sender, details.to, details.path[0], output, details.inputAmount, outputAmount, partnerId);
