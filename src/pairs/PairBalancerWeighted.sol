...
contract PairBalancerWeighted is ISwappaPairV1 {
    function swap(
        ...
        IPairBalancerWeightedVault vault = IPairBalancerWeightedVault(
            vaultAddr
        );
    ...
    function parseData(bytes memory data)
        public
        pure
        returns (address vaultAddr, bytes32 poolId)
    {
    ...
    function getAllPoolInfo(bytes memory data)
        public
        view
        returns (
            IPairBalancerWeightedVault,
            IPairBalancerBaseWeightedPool,
            ERC20[] memory,
            uint256[] memory,
            uint256[] memory
        )
    {
        ...
        IPairBalancerBaseWeightedPool pool = IPairBalancerBaseWeightedPool(
            poolAddr
        );

        (ERC20[] memory tokens, uint256[] memory balances, ) = vault
            .getPoolTokens(poolId);
        uint256[] memory weights = pool.getNormalizedWeights();
        return (vault, pool, tokens, balances, weights);
    }

    function getTokenBalanceWeightIndex(
        address tokenToFind,
        ERC20[] memory tokens,
        ...
        )
    {
        for (uint256 tokenIndex = 0; tokenIndex < tokens.length; tokenIndex++) {
            if (tokenToFind == address(tokens[tokenIndex])) {
                return (
                    (balances[tokenIndex], weights[tokenIndex], tokenIndex)
                );
            }
        }
    ...
    function calcNoFees(
        ...
        (uint256 balanceIn, uint256 weightIn, ) = getTokenBalanceWeightIndex(
            input,
            tokens,
            balances,
            weights
        );
        (uint256 balanceOut, uint256 weightOut, ) = getTokenBalanceWeightIndex(
            output,
            tokens,
            balances,
            weights
    ...
    function getOutputAmount(
        ...
        (
            ,
            IPairBalancerBaseWeightedPool pool,
            ERC20[] memory tokens,
            uint256[] memory balances,
            uint256[] memory weights
        ) = getAllPoolInfo(data);

        uint256 amountOut = calcNoFees(
            input,
            output,
            amountIn,
            tokens,
        ...
        return
            amountOut -
            WeightedMath.calcSwapFee(amountOut, pool.getSwapFeePercentage());
    }

    function getPoolSpecialization(bytes32 poolId)
        external
        pure
        returns (uint256 specialization)
    {
